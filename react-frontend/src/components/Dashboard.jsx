...
function Dashboard() {
  ...
  const fetchprojects = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.get("http://localhost:7000/project/list", {
        headers: {
  ...
  if (projectlist1) {
    console.log("This is from dashboard ",projectlist1[0]);
  }
  useEffect(() => {
    fetchprojects();
  }, []);
            ...
            {showform && (
              <div className="modal-overlay">
                <div className="modal-content">
                  <ProjectCreateform
                    closeform={() => (setshowform(false), fetchprojects())}
                  />
                </div>
              </div>
            )}
              ...
              {projectlist1.length === 0 ? (
                ...
                projectlist1.map((Element) => (
                  <div
                    key={Element._id}
                    className="card"
                    onClick={() => navigator(`/project/${Element._id}`)}
                  >
                    <h3>Project Name: {Element.projectname}</h3>
                    <p>Created By: {Element.createdBy.name}</p>
                    <p>Created At: {Element.startAt.split("T")[0]}</p>
                    <p>
                      {" "}
                      DeadLine:{" "}
                      {new Date(Element.deadLine).toISOString().split("T")[0]}
                    </p>
                    <div>
